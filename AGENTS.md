...
- 正常召回唯一入口 knowledge_recall，按当前授权范围匹配queries，资格去重后统一排序、8项/完整5000字符。Recall/Read事实失败仍交付且无本次稳定引用；Used按assetId累计，合法旧引用可结算；Hub/REST只读。
...
| 产品与实现契约 | [知识召回核心设计2.4](项目文档/设计方案/05-知识召回核心模型与Task解耦设计.md) 中的相关章节 |
