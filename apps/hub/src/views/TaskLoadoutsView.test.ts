...
describe("Task Loadouts view", () => {
  ...
  it("does not let an older list response replace a newer selection", async () => {
    ...
    const newerSummary = { ...summary, status: "CANCELLED" as const, taskId: "tsk2034512345678901299" };
    fetchMock.mockImplementation(async (input) => {
      ...
      if (path === `/api/task-loadouts/${newerSummary.taskId}`) {
        return jsonResponse({ ok: true, data: { taskLoadout: { ...detail, ...newerSummary } } });
      }
    ...
    const wrapper = await mountLoadedView();
    const status = wrapper.findAll(".filter-row select")[0];
    await status?.setValue("RUNNING");
    await wrapper.get("form").trigger("submit");
    await status?.setValue("CANCELLED");
    await wrapper.get("form").trigger("submit");
    await flushPromises();
    expect(wrapper.text()).toContain(newerSummary.taskId);

    oldResult.resolve(jsonResponse({ ok: true, data: { items: [{ ...summary, request: "Stale Task" }] } }));
    await flushPromises();
    expect(wrapper.text()).not.toContain("Stale Task");
  });

  it("handles a missing detail with a safe list refresh action", async () => {
    fetchMock.mockImplementation(async (input) => String(input).startsWith("/api/task-loadouts/")
  ...
  it.each([503, 500])("shows safe list recovery for HTTP %i", async (status) => {
    fetchMock.mockResolvedValue(jsonResponse({
      ok: false,
      error: { code: status === 503 ? "SERVICE_UNAVAILABLE" : "INTERNAL_ERROR", message: "private detail", retryable: status === 503 },
    }, status));
    const wrapper = await mountLoadedView();
  ...
  it("contains only GET-backed read controls", async () => {
    const wrapper = await mountLoadedView();
    expect(wrapper.text()).not.toMatch(/\b(Resolve|Attach|Complete Task|Cancel Task|Create|Edit|Delete|Promote|Confirm)\b/u);
    expect(fetchMock.mock.calls.every(([, init]) => init?.method === "GET")).toBe(true);
  });
});

async function mountLoadedView(): Promise<VueWrapper> {
  const wrapper = mount(TaskLoadoutsView);
  await flushPromises();
  await flushPromises();
  return wrapper;
}
