...
2. 同一知识目的已有原生检索词时，直接逐项放入 `queries`，保留原词；有上下文依据的补充表达只追加，不替换原词。没有原生检索词时，从当前 Request 和已有上下文提取相关关键词，并保留问题中的业务名称作为独立表达。同一知识目的的不同表达放入一次数组，项内 AND、项间 OR，不发送 `a|b`；独立知识问题分别召回。
3. 调用 `knowledge_recall({capabilityIds, queries})`。所有表达共用一次去重、排序和输出预算；有需要的知识未返回时，依据当前问题与已有线索补充查询，或通过已知 Asset ID 读取。
