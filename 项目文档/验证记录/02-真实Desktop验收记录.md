...
日期：2026-09-06。当前状态：**本轮本地验收结束，核心链路及两项故障恢复已实测，跨 Workspace／双窗口由用户豁免；后续放行复核已完成，用户已确认限定本机个人使用范围 GO。** 放行范围与已接受的验证限制见[本地使用放行复核](../审查记录/09-本地使用放行复核.md)。本轮实测结果见 §16–§20，不代表原清单全部实测通过；正文保留各历史时点的失败、未覆盖项和 NO-GO 结论。

依据：最终契约核对与 Desktop 验收清单（历史文档已清理）。本记录保存实际执行结果，不把准备脚本、SDK 或 CLI 验证当成真实 Desktop／模型验证。
